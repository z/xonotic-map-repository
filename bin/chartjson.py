...
def main():
    ...
    gametypes_sorted = OrderedDict(
        sorted(gametypes_dist.items(), key=itemgetter(1), reverse=True))
    c7['data']['json'] = gametypes_sorted

    # Donut (dupes)
    c8 = {'bindto': '', 'data': {'json': {}, 'type': 'donut'}}
    c8['bindto'] = '#chart-shacount'
    shadupes_sorted = OrderedDict(
        sorted(shadupes_dist.items(), key=itemgetter(1), reverse=True))
    ...
    entities_appearance_sorted = OrderedDict(
        sorted(entities_by_appearance.items(), key=itemgetter(1), reverse=True))
    ...
    entities_count_sorted = OrderedDict(
        sorted(entities_by_count.items(), key=itemgetter(1), reverse=True))
    ...
    fo = open(resources_dir + 'data/charts.json', 'w')
